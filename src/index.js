...
async function main(infile) {
    ...
    const zotobject = await jq.run(".[] | .successful | [ to_entries[] | .value.data ] ", result, { input: 'json', output: 'json' }).then(
        (data) => {
            return data;
        }
    );
    const openalexobject = await jq.run('.results | [ .[] | { "key": .id, "value": . } ] | from_entries', inob, { input: 'json', output: 'json' }).then(
        (data) => {
            return data;
        }
    );
